...
from utils.experiment_logger import create_experiment_folder_real, save_config_summary
...
config = {
    ...
    "file_mode": "chat_control",            # 'grab_mec', 'grab_control', 'chat_control'
    ...
    "spatial_dim": "width",             # Collapse along 'width' or 'height'
    "time_average": True,               # Average across time steps within segment

    # Optimizer settings
    "optimizer": "nag",                  # 'gd', 'momentum', 'nag', "gd_torch", "momentum_torch" , "nag_torch"
    "eta": ETA,
    "lambda": LAMBDA,
    "max_iters": MAX_ITERS,
    ...
    # File selection
    "file_subset": "file1_only",               # e.g., 'file1_only', 'file2_only', 'file3_only', 'all'; -> only for chat_control
}

# --- Create Experiment Folder ---
experiment_path = create_experiment_folder_real(config)
...
# --- Save Final Loss and Time Taken ---
final_loss = loss[-1]
with open(f"{experiment_path}/metrics.txt", "a") as f:
    f.write(f"Final Loss: {final_loss:.6f}\n")
    f.write(f"Runtime: {runtime:.4f} seconds\n")

# --- Visualization & Save ---
plot_results(Z, gamma, loss, title="γ(t) over Z(x,t) - Real Data", save_dir=experiment_path, filename="result.png")

# --- Post-Hoc RMSE Evaluation Against dF/F₀ (if available) --- . outdated since dfof does not represent ground truth in this context
...
if dfof_signal:
    ...
    plt.title("Raw dF/F₀ Signal")
    plt.xlabel("Time")
    plt.ylabel("dF/F₀")
    plt.savefig(f"{experiment_path}/dfof.png")
    plt.show()
